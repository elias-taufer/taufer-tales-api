...
public class TaleService {
    ...
    public TaleResponse get(Long id) {
        var t = tales.findById(id).orElseThrow();
        return new TaleResponse(t.getId(), t.getTitle(), t.getAuthor(), t.getIsbn(), t.getDescription(), t.getCoverUrl(), t.getPublishedYear(), t.getTags(), reviews.avgRating(id));
    }

    public TaleResponse create(TaleCreateDto d) {
        var t = tales.save(Tale.builder().title(d.title()).author(d.author()).isbn(d.isbn()).description(d.description()).coverUrl(d.coverUrl()).publishedYear(d.publishedYear()).tags(d.tags()).build());
        return get(t.getId());
    }
