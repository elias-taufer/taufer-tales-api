...
import com.taufer.tales.domain.*; import com.taufer.tales.dto.*; import com.taufer.tales.repo.*;
import lombok.RequiredArgsConstructor; import org.springframework.security.core.Authentication; import org.springframework.stereotype.Service;
import java.time.Instant; import java.util.List;

@Service @RequiredArgsConstructor
public class ReviewService {
  private final ReviewRepository reviews; private final TaleRepository tales; private final UserRepository users;
  ...
  public ReviewResponse update(Long id, ReviewUpdateDto d, Authentication auth){
    ...
    r.setRating(d.rating()); r.setTitle(d.title()); r.setBody(d.body()); return new ReviewResponse(reviews.save(r).getId(), r.getTale().getId(), r.getUser().getUsername(), r.getRating(), r.getTitle(), r.getBody(), r.getCreatedAt().toString());
  }
  public void delete(Long id, Authentication auth){ Review r = reviews.findById(id).orElseThrow(); if(!r.getUser().getUsername().equals(auth.getName())) throw new RuntimeException("forbidden"); reviews.delete(r); }
}
